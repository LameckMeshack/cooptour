defmodule CooptourWeb.UserAuthTest do
  use CooptourWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias Cooptour.Accounts
  alias Cooptour.Accounts.Scope
  alias CooptourWeb.UserAuth

  import Cooptour.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, CooptourWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      CooptourWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        CooptourWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
